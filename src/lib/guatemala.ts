...
export const departamentosGuatemala: Record<string, Departamento> = {
  'alta-verapaz': {
    nombre: 'Alta Verapaz',
    municipios: [
...
// Lista de departamentos ordenados alfabéticamente para los selects
export const listaDepartamentos = Object.entries(departamentosGuatemala)
  .map(([key, value]) => ({
    value: key,
    label: value.nombre
  }))
...
export function getMunicipios(departamentoKey: string): string[] {
  return departamentosGuatemala[departamentoKey]?.municipios || []
}

// Función para obtener el nombre del departamento
export function getNombreDepartamento(departamentoKey: string): string {
  return departamentosGuatemala[departamentoKey]?.nombre || ''
}
